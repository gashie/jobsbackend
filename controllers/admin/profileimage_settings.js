...
exports.UpdateUserDataWithImage = asynHandler(async (req, res, next) => {
    let actor = req.user.userInfo
    ...
    if (result.affectedRows === 1) {
        CatchHistory({ event: 'Update user profile', functionName: 'UpdateUserDataWithImage', response: `user profile record with id ${actor.userId} was updated by ${actor.userId}`, dateStarted: req.date, state: 1, requestStatus: 200, actor: actor.userId }, req);
        return sendResponse(res, 1, 200, 'Record Updated')

    } else {
        CatchHistory({ event: 'Update user profile', functionName: 'UpdateUserDataWithImage', response: `Error Updating Record with id ${actor.userId}`, dateStarted: req.date, state: 0, requestStatus: 200, actor: actor.userId }, req);
        return sendResponse(res, 0, 200, 'Error Updating Record')
    }
